...
/**
 * DeepResearch Agent 提示词
 * <p>
 * 包含深度研究各阶段的系统提示词：需求澄清、研究主题生成、
 * 执行计划规划、工具执行、质量评审、上下文压缩、最终报告总结。
 * 移植自 dodo-agent 的 PlanExecutePrompts。
 * </p>
 */
public final class DeepResearchPrompts {
     ...
     * 获取当前系统时间
     * <p>
     * 时间信息作为独立的上下文注入，解决 LLM 训练数据截止导致的日期认知错误。
     * </p>
     */
     ...
     * 需求澄清提示词
     * <p>
     * 用于判断用户的问题是否信息充足，可以开始研究。
     * 输出包含【开始研究】或【需要补充信息】标记。
     * </p>
     */
     ...
     * 执行计划规划提示词
     * <p>
     * Plan 阶段使用，将研究主题拆解为搜索工具调用任务。
     * 输出必须是严格 JSON 数组，每个元素包含 id、instruction、order。
     * </p>
     */
     ...
     * 工具执行提示词
     * <p>
     * Execute 阶段的子 ReAct Agent 使用，负责忠实整理工具返回结果。
     * </p>
     */
     ...
     * 质量评审提示词
     * <p>
     * Critique 阶段使用，判断当前研究结果是否足以支持最终报告输出。
     * 输出必须是严格 JSON：{"passed": true|false, "feedback": "..."}
     * </p>
     */
     ...
     * 最终报告总结提示词
     * <p>
     * Summarize 阶段使用，基于工具检索结果生成深度研究分析报告。
     * </p>
     */
    ...
    /**
     * 子 Agent（mini ReAct loop）系统提示词
     * <p>
     * 用于 Execute 阶段内部的工具调用循环。
     * </p>
     */
    public static final String REACT_SYSTEM_PROMPT = """
            ...
            4. 如果上下文已经包含了完成任务的全部信息，则不要再调用任何工具。
            """;
}
